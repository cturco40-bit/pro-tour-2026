...
- The field is a locked record at Firebase `playoffField` (`playoffField` global, `savePlayoffField()`). Admin locks it from Setup → Playoff 1; that lock is what **issues** start strokes. **Nothing start-stroke related renders until it is locked** — not the board, not the per-player START pills — so the un-locked state has to stay explicit rather than rendering empty.
- **Two lock paths, and groups-first is the normal one.** The commissioner builds the Playoff 1 groups by hand in standings order, so `lockPlayoffFieldFromGroups()` seeds straight off that running order (group 1 slot 1 = seed 1) via `playoffOrderFromGroups()`, which also queues everyone else as alternates. `lockPlayoffField()` seeds off `calcPlayoffSeeds()` instead, and is only offered when no groups exist — note it returns nothing until all 8 regular rounds are submitted, so it is useless mid-season. The admin panel picks the groups-first path whenever Playoff 1 groups exist.
