...
The playoff advantage is a **start-stroke adjustment to the score, never a handicap change**. `Playoff net = gross − course handicap + startStrokes`.

- It is a **plain per-player number the commissioner types**, in an `ADV` box next to the handicap box in Round Setup → Playoff 1, and it lives on the group's player record beside the handicap: `{name, hcp, startStrokes}`. That record is the **one source of truth** — `getStartStrokes()` reads it and derives nothing.
- **Do not reintroduce derivation.** An earlier version computed this from a separate locked seeding record, and every bug it produced was the same bug: the number shown somewhere disagreed with the number the commissioner meant (invisible UI gated on a lock, an admin panel seeded off standings that never matched the hand-built groups, a 13th-place call-up priced by group position instead of standings). Seeds are the *convention*, not the mechanism.
- `startStrokesForSeed(pos)` encodes the convention — 1–4 → −2, 5–8 → −1, 9–12 → 0, 13th/14th/15th → +2/+3/+4 — and backs the "Fill by Group" button (`fillPlayoffStartStrokes()`), which writes into the inputs only. Nothing is saved until Save Groups, and a typed value always wins.
- A mid-season swap is just "change the name, type the number". There is no withdrawal/alternate/lock machinery to keep in step.
- `hasStartStrokes(round)` gates the board and the per-player pills — they appear as soon as a value is entered, with no separate lock step.
- Applies to **Playoff 1 only** and rides into the combined total; the Championship re-applies nothing. `getCarryIn()` supplies each player's Playoff 1 net as their Championship starting score, and is applied **only** at the three live-board sites — never inside `getEffectiveNvp`, since `buildPlayoffCombined` sums r1 + r2 itself and would double-count round 1.
...
- **R1 / R2 / Tot are each round's own net plus the combined** — on both the champion card and Standings → Playoffs. R2 was once a running total, which restated Tot in two columns and hid how a player got there (two players level on the total off different second rounds looked identical). The **live** leaderboard still carries round 1 forward while the Championship is being played — that's `getCarryIn()` at the three live-board sites, a different surface. Don't unify them.
